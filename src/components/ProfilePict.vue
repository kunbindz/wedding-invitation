<template>
  <section 
    ...
    <div data-aos="slide-up" class="mt-10 w-full text-center text-slate-800 text-sm">
      <p class="text-orange-100 text-[2rem] great-vibes tangerine-font py-2">{{ source.name }}</p>
      <p class="text-orange-100 mb-3 mt-1">{{ source.sex }} {{ source.parents[0] }} {{ source.parents[1] }}</p>
      <p class="text-orange-100">{{ source.info }}</p> 
